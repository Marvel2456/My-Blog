...
class EmailValidationView(View):
    def post(self, request):
        ...
        if User.objects.filter(email=email).exists():
            return JsonResponse({'email_error': 'Sorry email is in use, choose another username'}, status=409)
...
class UsernameValidationView(View):
    def post(self, request):
        ...
        if not str(username).isalnum():#if username is not alphanumeric
            return JsonResponse({'username_error': 'username should contain only alphanumeric characters'}, status=400)
        if User.objects.filter(username=username).exists():#if username exist
